...
\begin{definition}
    ...
    % TODO: fix this (defn of fibration, edge case leaves)
\end{definition}

% TODO: K_F=O(-1) implies pencil of lines in tangency orders section
We saw a particular example of some converse to this result in blah. There is a
general statement which can be made:
...
\begin{proof}[Sketch proof]
    ...
    $\calT_X\subseteq\Hilb_X$. By assumption, the canonical map from the
    universal family $\calU$ over $\calT_X$ to $X$ is dominant. Since $\calU$ is
    an infinite disjoint union of projective components, by the Baire category
    theorem we have some such component $Z\subset\calU$ over $Y\subset\calT_X$
    which maps surjectively to $X$. After restricting to a suitable hyperplane
    $Y^0$ in $Y$, we get a finite surjection $Z^0\to X$, which lifts to a Galois
    covering $\tilde Z^0\to X$, where the Galois group $G$ can be made to act on
    a base $\tilde Y^0$.
    \begin{equation*}
        \begin{tikzcd}
            \tilde Z^0 \ar[d] \ar[r]
